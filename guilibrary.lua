...
-- FIX: Safe callback wrapper to prevent UI from breaking if user script errors
local function safeCall(cb, ...)
    if type(cb) == "function" then
        local ok, err = pcall(cb, ...)
        if not ok then
            warn("[KreinHub] Callback Error:", err)
        end
    end
end
...
-- FIX: Global lock so only ONE slider/colorpicker can be dragged at a time
local _globalSliderDragging = false
 ...
 function WindowObj:CreateTab(name, icon)
     ...
     function TabObj:CreateSection(sTitle, collapsed)
             ...
             action.MouseButton1Click:Connect(function()
                 ...
                 if confirm then
                     if not _conf then
                         ...
                         tw(action, {BackgroundColor3 = bg}, 0.18); safeCall(cb)
                     end
                 else
         ...
         function SObj:CreateToggle(text, default, cb)
             ...
             hb.MouseButton1Click:Connect(function() _v = not _v; apply(_v, true); safeCall(cb, _v) end)
             local obj = {}
             function obj:SetValue(v) _v = v==true; apply(_v, true); safeCall(cb, _v) end
         ...
         function SObj:CreateSlider(text, mn, mx, def, cb, float)
             ...
             valBox.FocusLost:Connect(function()
                 ...
                 if typed then
                     ...
                     valBox.Text=fmtVal(_v); safeCall(cb, _v)
             ...
             local function drag(inp)
                 ...
                 local function cleanup()
                     ...
                     if mc then pcall(function() mc:Disconnect() end) end
                     if ec then pcall(function() ec:Disconnect() end) end
                     tw(knob, {Size = UDim2.new(0,16,0,16)}, 0.1)
                 end

                 local function update(pos)
                     if not bar.Parent then cleanup() return end
                 ...
                 mc = UIS.InputChanged:Connect(function(mi)
                     if mi.UserInputType == Enum.UserInputType.Touch or mi.UserInputType == Enum.UserInputType.MouseMovement then
                         update(mi.Position)
                     end
                 end)

                 ec = UIS.InputEnded:Connect(function(ei)
                     if ei.UserInputType == Enum.UserInputType.Touch or ei.UserInputType == Enum.UserInputType.MouseButton1 then
                         cleanup()
                     end
                 end)
             end

             knob.InputBegan:Connect(drag)
             bar.InputBegan:Connect(drag)
         ...
         function SObj:CreateDropdown(config, _legacyOpts, _legacyCb, _legacyMulti)
                 ...
                 for i,opt in ipairs(list) do
                     ...
                     ob.MouseButton1Click:Connect(function()
                         if multi then
                             ...
                             for o,v in pairs(_sel) do if v then table.insert(sel,o) end end; safeCall(cb, _sel, sel)
                         else _sel=opt; selLbl.Text=opt; closeDropdown(); safeCall(cb, opt) end
                     end)
                 end
             end
             buildOpts(opts)
             ...
             function obj:SetValue(v)
                 if multi then
                     ...
                     local sel={}; for o,val in pairs(_sel) do if val then table.insert(sel,o) end end; safeCall(cb, _sel, sel)
                 else if table.find(opts,v) then _sel=v; selLbl.Text=v; safeCall(cb, v) end end
             end
         ...
         function SObj:CreateKeybind(text, defKey, cb)
             ...
             kBtn.MouseButton1Click:Connect(function()
                 ...
                 _kconn=UIS.InputBegan:Connect(function(inp,gp)
                     ...
                     tw(kBtn,{BackgroundColor3=C.KeybindBg},0.12); stopBind(); safeCall(cb, _key)
                 end)
                 reg(_kconn)
             end)
             local obj={}
             function obj:SetValue(k) _key=k; kBtn.Text=k.Name; safeCall(cb, k) end
         ...
         function SObj:CreateColorPicker(text, defColor, cb)
             ...
             local function dragSV(inp)
                 if not svFrame.Parent then return end
             ...
             svBtn.InputBegan:Connect(function(inp)
                 if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
                     ...
                     mc = UIS.InputChanged:Connect(function(mi)
                         if mi.UserInputType == Enum.UserInputType.Touch or mi.UserInputType == Enum.UserInputType.MouseMovement then dragSV(mi) end
                     end)
                     ec = UIS.InputEnded:Connect(function(ei)
                         if ei.UserInputType == Enum.UserInputType.Touch or ei.UserInputType == Enum.UserInputType.MouseButton1 then cleanup() end
                     end)
                 end
             end)
             hueBtn.InputBegan:Connect(function(inp)
                 if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
                     ...
                     mc = UIS.InputChanged:Connect(function(mi)
                         if mi.UserInputType == Enum.UserInputType.Touch or mi.UserInputType == Enum.UserInputType.MouseMovement then dragHue(mi) end
                     end)
                     ec = UIS.InputEnded:Connect(function(ei)
                         if ei.UserInputType == Enum.UserInputType.Touch or ei.UserInputType == Enum.UserInputType.MouseButton1 then cleanup() end
                     end)
         ...
         function SObj:CreateTextBox(labelText, ph, cb, config)
             ...
             inp:GetPropertyChangedSignal("Text"):Connect(function()
                 ...
                 _filtering=false; if realTime then safeCall(cb, inp.Text) end
             end)
             ...
             inp.FocusLost:Connect(function(enterPressed)
                 ...
                 if not realTime or enterPressed then safeCall(cb, inp.Text) end
             end)
             local obj={}
         ...
         function SObj:CreateRadioGroup(config, _legacyDefault, _legacyCb, _legacyMulti)
             ...
             for i,option in ipairs(options) do
                 ...
                 btn.MouseButton1Click:Connect(function()
                     if multi then
                         ...
                         safeCall(cb, _selected, selectedOpts)
                     else
                         _selected=i
             ...
             function obj:SetValue(index)
                 if multi then
                     ...
                     safeCall(cb, index, options[index])
                 end
             end
